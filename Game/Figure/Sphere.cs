...
namespace Game.Figure
{
    class Sphere: Model, IModel
    {
        ...
        public Sphere()
        {
            ...
            p1 = new DenseVector(new double[] { R * Cos(Alpha) * Cos(Beta), R * Sin(Alpha) * Cos(Beta), R * Sin(Beta), 1 });
            p2 = new DenseVector(new double[] { R * Cos(Alpha) * Cos(Beta + Angle), R * Sin(Alpha) * Cos(Beta + Angle), R * Sin(Beta + Angle), 1 });
            while (Beta <= PI / 2)
            {
                while (Alpha <= PI * 2)
                {
                    p3 = new DenseVector(new double[] { R * Cos(Alpha + Angle) * Cos(Beta), R * Sin(Alpha + Angle) * Cos(Beta), R * Sin(Beta), 1 });
                    p4 =  new DenseVector(new double[] { R * Cos(Alpha + Angle) * Cos(Beta + Angle), R * Sin(Alpha + Angle) * Cos(Beta + Angle), R * Sin(Beta + Angle), 1 });
